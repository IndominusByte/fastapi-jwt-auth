...
async def test_verified_token(client, encoded_token, Authorize):
    ...
    access_token = await Authorize.create_access_token(subject='test')
...
async def test_get_raw_token(client, default_access_token, encoded_token):
    response = client.get('/raw_token', headers={"Authorization": f"Bearer {encoded_token}"})
    assert response.status_code == 200
    assert response.json() == default_access_token


async def test_get_raw_jwt(default_access_token, encoded_token, Authorize):
    assert await Authorize.get_raw_jwt(encoded_token) == default_access_token


async def test_get_jwt_jti(client, default_access_token, encoded_token, Authorize):
    assert await Authorize.get_jti(encoded_token=encoded_token) == default_access_token['jti']


async def test_get_jwt_subject(client, default_access_token, encoded_token):
...
async def test_invalid_jwt_issuer(client, Authorize):
...
async def test_valid_aud(client, Authorize, token_aud):
    AuthJWT._decode_audience = ['foo', 'bar']

    access_token = await Authorize.create_access_token(subject=1, audience=token_aud)
    response = client.get('/protected', headers={'Authorization': f"Bearer {access_token}"})
    assert response.status_code == 200
    assert response.json() == {'hello': 'world'}

    refresh_token = await Authorize.create_refresh_token(subject=1, audience=token_aud)
    response = client.get('/refresh_token', headers={'Authorization': f"Bearer {refresh_token}"})
    assert response.status_code == 200
...
async def test_invalid_aud_and_missing_aud(client, Authorize, token_aud):
...
async def test_invalid_decode_algorithms(client, Authorize):
...
async def test_valid_asymmetric_algorithms(client, Authorize):
    hs256_token = await Authorize.create_access_token(subject=1)
    ...
    rs256_token = await Authorize.create_access_token(subject=1)
...
async def test_invalid_asymmetric_algorithms(client, Authorize):
    ...
    with pytest.raises(RuntimeError, match=r"authjwt_private_key"):
        await Authorize.create_access_token(subject=1)
